...
class RandomDropout(nn.Module):
    ...
    def forward(self, input):
        bs, dm, w, h = input.shape
        c_num = w*h
        input = input.view(bs, dm, c_num)
        full_mask = torch.ones(bs, c_num).cuda()
        select_mask = full_mask

        #
        mask = torch.rand((bs, c_num))
        select_mask[mask < self.mask_rate] = 0
        #
        att_output = torch.einsum('bac,bc->bac', [input, select_mask])
        return att_output.mean(-1)
...
class SubResNet2(nn.Module):

    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
                 dropblock_size=5, use_se=False):
        super(SubResNet2, self).__init__()
...
class SubResNet3(nn.Module):

    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
                 dropblock_size=5, use_se=False):
        super(SubResNet3, self).__init__()
        ...
        self.layer2 = self._make_layer(block, n_blocks[1], 160,
                                       stride=2, drop_rate=drop_rate)
        self.layer3 = self._make_layer(block, n_blocks[2], 640,
...
class SubResNet4(nn.Module):

    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
                 dropblock_size=5, use_se=False):
        super(SubResNet4, self).__init__()
...
def subresnet12(keep_prob=1.0, avg_pool=False, **kwargs):
    """Constructs a ResNet-12 model.
    """
    model1 = SubResNet1(BasicBlock, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
    model2 = SubResNet2(BasicBlock, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
    model3 = SubResNet3(BasicBlock, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
    model4 = SubResNet4(BasicBlock, [1, 1, 1, 1], keep_prob=keep_prob, avg_pool=avg_pool, **kwargs)
